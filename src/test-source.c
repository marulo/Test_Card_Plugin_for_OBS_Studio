...
static void render_text_cached(text_cache_t *cache, const char *text, stbtt_fontinfo *font, int font_size) {
  ...
  if (cache->bitmap) bfree(cache->bitmap);
  ...
  for (const char *p = text; *p; p++) {
    ...
    if (glyph_x >= 0 && glyph_y >= 0 && glyph_x + glyph_w <= cache->width && glyph_y + glyph_h <= cache->height) {
...
static void blend_text_to_buffer(uint32_t *buffer, int buf_width, int buf_height, text_cache_t *cache, int x, int y, uint32_t color) {
  if (!cache->valid || !cache->bitmap) return;
  ...
  for (int row = 0; row < cache->height; row++) {
    int dest_y = y + row;
    if (dest_y < 0 || dest_y >= buf_height) continue;
    for (int col = 0; col < cache->width; col++) {
      int dest_x = x + col;
      if (dest_x < 0 || dest_x >= buf_width) continue;
...
static void resize_grid(struct test_source_data *data) {
  if (data->width == 0 || data->height == 0 || data->cell_size <= 0) return;
  ...
  int max_cell_y = (int)floor((double)(data->height - 1 - cy) / data->cell_size);
  ...
  if (data->grid_colors && data->grid_cols == cols && data->grid_rows == rows) return;

  if (data->grid_colors) bfree(data->grid_colors);
  if (data->grid_cell_timers) bfree(data->grid_cell_timers);
  if (data->grid_random_targets) bfree(data->grid_random_targets);
...
static void update_grid_texture(struct test_source_data *data) {
  if (!(data->dirty & DIRTY_GRID)) return;
  if (!data->grid_colors || data->grid_cols == 0 || data->grid_rows == 0) return;

  obs_enter_graphics();
  if (!data->tex_grid) {
      data->tex_grid = gs_texture_create(data->grid_cols, data->grid_rows, GS_RGBA, 1, NULL, GS_DYNAMIC);
  }
  
  gs_texture_set_image(data->tex_grid, (const uint8_t *)data->grid_colors, data->grid_cols * 4, false);
  obs_leave_graphics();
  data->dirty &= ~DIRTY_GRID;
}

static void render_layer_text(struct test_source_data *data) {
  if (!(data->dirty & DIRTY_TEXT)) return;
  ...
  if (data->cache_resolution.width > max_w) max_w = data->cache_resolution.width;
  if (data->cache_clock.width > max_w) max_w = data->cache_clock.width;
  
  int outline = 3;
  int total_h = data->cache_title.height + padding + data->cache_resolution.height + padding + data->cache_clock.height;
  ...
  if (data->text_tex_width != tex_w || data->text_tex_height != tex_h) {
      if (data->text_buffer) bfree(data->text_buffer);
      data->text_buffer = bmalloc(tex_w * tex_h * sizeof(uint32_t));
  ...
  memset(data->text_buffer, 0, tex_w * tex_h * sizeof(uint32_t));
  ...
  for (int dy = -outline; dy <= outline; dy++) {
    for (int dx = -outline; dx <= outline; dx++) {
      ...
      if (dx * dx + dy * dy > outline * outline + 1) continue; // Round outline
      blend_text_to_buffer(data->text_buffer, tex_w, tex_h, &data->cache_title, title_x + dx, title_y + dy, outline_color);
      blend_text_to_buffer(data->text_buffer, tex_w, tex_h, &data->cache_resolution, res_x + dx, res_y + dy, outline_color);
      blend_text_to_buffer(data->text_buffer, tex_w, tex_h, &data->cache_clock, clock_x + dx, clock_y + dy, outline_color);
    }
  }
  ...
  obs_enter_graphics();
  if (!data->layer_text) {
      data->layer_text = gs_texture_create(tex_w, tex_h, GS_RGBA, 1, NULL, GS_DYNAMIC);
  }
  gs_texture_set_image(data->layer_text, (const uint8_t *)data->text_buffer, tex_w * 4, false);
  obs_leave_graphics();
  data->dirty &= ~DIRTY_TEXT;
}

static void draw_solid_rect(gs_effect_t *effect, gs_texture_t *white_tex, float x, float y, float w, float h, uint32_t color) {
    if (!white_tex) return;
...
static void test_source_destroy(void *data) {
  ...
  if (src->text_buffer) bfree(src->text_buffer);

  if (src->cache_clock.bitmap) bfree(src->cache_clock.bitmap);
  if (src->cache_resolution.bitmap) bfree(src->cache_resolution.bitmap);
  if (src->cache_title.bitmap) bfree(src->cache_title.bitmap);

  if (src->font_data) bfree(src->font_data);

  if (src->grid_colors) bfree(src->grid_colors);
  if (src->grid_cell_timers) bfree(src->grid_cell_timers);
  if (src->grid_random_targets) bfree(src->grid_random_targets);

  obs_enter_graphics();
  if (src->tex_white) gs_texture_destroy(src->tex_white);
  if (src->tex_grid) gs_texture_destroy(src->tex_grid);
...
static void test_source_video_tick(void *data, float seconds) {
  ...
  if (src->h_scan_time >= 40.0f) src->h_scan_time -= 40.0f;
  src->v_scan_time += seconds;
  if (src->v_scan_time >= 20.0f) src->v_scan_time -= 20.0f;
  ...
  if (src->grid_colors && src->grid_cell_timers && src->grid_random_targets) {
    ...
    for (int i = 0; i < total_cells; i++) {
      if (src->grid_cell_timers[i] > 0.0f) {
        ...
        if (src->grid_cell_timers[i] < 0.0f) src->grid_cell_timers[i] = 0.0f;
...
static void test_source_video_render(void *data, gs_effect_t *effect) {
  ...
  if (src->tex_grid && src->point_effect) {
      ...
      int tex_w = src->grid_cols * src->cell_size;
      int tex_h = src->grid_rows * src->cell_size;
  ...
  if (src->layer_text) {
      ...
      gs_technique_begin_pass(tech_def, 0);
      gs_draw_sprite(src->layer_text, 0, src->text_tex_width, src->text_tex_height);
      gs_technique_end_pass(tech_def);
